...
declare type express$Path = string | RegExp | Array<string | RegExp>;
...
declare class express$Response extends http$ServerResponse mixins express$RequestResponseBase {
  ...
  render(view: string, locals?: {[name: string]: any}, callback?: express$RenderCallback): this;
  ...
  req: express$Request;
  // adding expose from express-state;
  expose<T>(T, namespace: string, options?: { cache?: boolean, isJSON?: boolean }): this;
}
...
declare interface express$Get<T> {
  (name: string): any;
  (middleware: express$Middleware): T;
  (...middleware: Array<express$Middleware>): T;
  (path: express$Path|express$Path[], ...middleware: Array<express$Middleware>): T;
